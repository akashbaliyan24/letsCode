...
const ProblemPage = () => {
  ...
  // const submissionArray = useSubmissionStore(state => state.submissions)
  ...
  useEffect(() => {
    if (submissions.length > 0) {
      ...
      const rate = ((acceptedCount / total) * 100).toFixed(2);
      setSuccessRate(rate);
    } else {
      setSuccessRate(null);
    }
      ...
      case "description":
        return (
          <div className="prose max-w-none">
            <p className="txt-lg mb-6">{problem.description}</p>
            {problem.examples && (
              <>
                <h3 className="text-xl font-bold mb-4">Examples:</h3>
                {Object.entries(problem.examples).map(([lang, example], idx) => (
                  <div className="bg-base-200 p-6 rounded-xl mb-6 font-mono" key={lang}>
                    <div className="mb-4">
        ...
        <div className="flex-1 gap-2">
          ...
          <div className="mt-2">
            ...
            <div className="flex items-center gap-2 text-sm text-base-content/70 mt-5 ">
              ...
              <span>{submissionCount} Sumbmissions</span>
              <span className="text-base-content/30">•</span>
              <ThumbsUp className='w-4 h-4' />
              <span>
                Success Rate: {successRate !== null ? `${successRate}%` : "No submissions yet"}
              </span>
            </div>
          </div>
        </div>
        <div className="flex-none gap-4 ">
          <button className={`btn btn-ghost btn-circle ${isBookmarked ? "text-primary" : ""
          ...
          <select className="select select-bordered select-primary w-40"
              ...
              <div className="p-4 border-t border-base-300 bg-base-200">
                <div className="flex justify-between items-center">
                  <button className={`btn btn-primary gap-2 ${isExecuting ? "loading" : ""
                    }`} disabled={isExecuting} onClick={handleRunCode} >
