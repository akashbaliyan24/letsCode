...
const ProfileSubmission = () => {
    ...
    const[expendedSubmission,setExpendedSubmission] = useState(null)
    ...
    const toggleExpand = (id) => {
      if(expendedSubmission === id){
        setExpendedSubmission(null);
      } else{
        setExpendedSubmission(id);
      }
    };
    ...
    <div className="n bg-base-200 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-primary mb-4 md:mb-0">My Sumbmissions</h1>

          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <div className="dropdown dropdown-end">
              <div className="btn btn-outline gap-2" tabIndex={0} role='button'>
                <Filter size={16}/>
                {filter === "all" ? "All Submissions" : filter}
              </div>
              <ul className="dropdown-content z-10 menu p-2 shadow bg-base-100 rounded-box w-52" tabIndex={0}>
                <li><button onClick={()=> setFilter('all')}>All Submissions</button></li>
                <li><button onClick={()=>setFilter('ACCEPTED')}>Accepted</button></li>
                <li><button onClick={()=>setFilter('WRONG_ANSWER')}>Wrong Answer</button></li>
        ...
        {filteredSubmissions.length === 0 ? (
          <div className="card bg-base-100 shadow-xl ">
            <div className="card-body items-center text-center">
              ...
              <p>You haven't submitted any solution , or none matches your current filter .</p>
            </div>
          </div>
        ) : (
          <div className="space-y-6">
            {filteredSubmissions.map((submission)=> (
              <div className="card bg-base-100 shadow-xl overflow-hidden transition-all duration-300" key={submission.id}>
                <div className="card-body p-0" role='button' onClick={()=> toggleExpand(submission.id)}>
                  {/* Submission Header  */}
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 cursor-pointer hover:bg-base-200">
                    ...
                    <div className="flex items-center gap-2 mt-3 md:mt-0">
                      {expendedSubmission === submission.id ? (
                        <ChevronUp size={20}/>
                      ) : (
                        <ChevronDown size={20}/>
                      )}
                        ...
                        <div className="stats shadow">
                          <div className="stat">
                            ...
                            <div className="stat-value text-lg">
                              ...
                              ? JSON.parse(submission.time)[0]:submission.time || 'N/A'}
                            </div>
                          </div>
                        </div>
                        <div className="stats shadow">
                          <div className="stat">
                            ...
                            <div className="stat-value text-lg">
                              ...
                              ?JSON.parse(submission.memory)[0] :submission.memory || 'N/A'}
                            </div>
                          </div>
                        </div>
                      </div>
